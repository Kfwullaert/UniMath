...
Section RezkAssociator.
  ...
  Proof.
    ...
    3: {
      ...
      apply (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff _)).
    }
    use nat_z_iso_comp.
    ...
    use pair_nat_z_iso.
    - apply (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC)).
    - apply functor_commutes_with_id.
  Defined.

  Lemma TransportedAssocLeftOnOb (x y z : C)
    : TransportedAssocLeft ((x,y),z)
      = # TD (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (x,y) #, identity (H z))
          · (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (TC (x,y) , z)).
  Proof.
    cbn.
    rewrite ! id_left.
    rewrite ! id_right.
  ...
  Lemma TransportedAssocLeftInvOnOb (x y z : C)
    : nat_z_iso_inv TransportedAssocLeft ((x,y),z)
  ...
  Proof.
    ...
    3: {
      apply pre_whisker_nat_z_iso.
      apply nat_z_iso_inv, TransportedTensorComm.
    }
  ...
  Lemma TransportedAssocRightOnOb (x y z : C)
    : TransportedAssocRight ((x,y),z)
      =  # TD (# H (id x) #, nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (y, z))
           · (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC)) (x, TC (y,z)).
  Proof.
    cbn.
    rewrite ! id_left.
    rewrite ! id_right.
  ...
  Lemma TransportedAssocRightInvOnOb (x y z : C)
    : nat_z_iso_inv TransportedAssocRight ((x,y),z)
      = (TransportedTensorComm Duniv H_eso H_ff TC)
          (x, TC (y,z)) · # TD (# H (id x) #, (TransportedTensorComm Duniv H_eso H_ff TC (y, z))).
  Proof.
    use (z_iso_inv_to_left _ _ _
                           (_,,pr2 (TransportedTensorComm Duniv H_eso H_ff TC) (x, TC (y,z)))).
    ...
    2: {
      ...
      rewrite id_right.
      apply maponpaths.
      apply (! pr22 ((pr2 (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC))) (y, z))).
    }
    etrans.
    2: {
      apply maponpaths_2.
  ...
  Definition TransportedAssociator
    : associator TD.
  Proof.
    use (lift_nat_trans_along (_,,Duniv) HHH HHH_eso HHH_ff).
    use nat_trans_comp.
    2: apply ((TransportedAssocRight)).
    use nat_trans_comp.
    3: exact (nat_z_iso_inv TransportedAssocLeft).
    exact (post_whisker α H).
  Defined.
  ...
  Definition TransportedAssociatorEq
    : pre_whisker HHH TransportedAssociator
      = nat_trans_comp _ _ _
          (nat_trans_comp _ _ _ TransportedAssocRight (post_whisker α H))
          (nat_z_iso_inv (TransportedAssocLeft)).
  Proof.
    set (t := lift_nat_trans_along_comm (_,,Duniv) _ HHH_eso HHH_ff
          (nat_trans_comp _ _ _ TransportedAssocRight
                          (nat_trans_comp _ _ _
                             (post_whisker α H)
                             (nat_z_iso_inv TransportedAssocLeft)
                          )
    ...
    exact (nat_trans_comp_assoc (homset_property _)
                                     _ _ _ _
              (pr1 TransportedAssocRight)
              (post_whisker α H : functor_composite (assoc_right TC) H ⟹ functor_composite (assoc_left TC) H)
              (nat_z_iso_inv TransportedAssocLeft)).
  Qed.

  Definition TransportedAssociatorOnOb
    : ∏ x : (C ⊠ C) ⊠ C,
        αD (HHH x) = TransportedAssocRight x · #H (α x) · nat_z_iso_inv TransportedAssocLeft x.
  Proof.
  ...
  Proof.
    ...
    use (# TD).
    use catbinprodmor.
    1: exact (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (x1,x2)).
    apply identity.
  Defined.
  ...
  Proof.
    ...
    use catbinprodmor.
    1: apply identity.
    apply (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (x2,x3)).
  Defined.
  ...
  Proof.
    ...
    use catbinprodmor.
    - apply identity.
    - exact (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (x3,x4)).
  Defined.
  ...
  Proof.
    ...
    use catbinprodmor.
    2: apply identity.
    exact ((TransportedTensorComm Duniv H_eso H_ff TC) (x1,x2)).
  Defined.
    ...
    3: {
      ...
      2: {
        ...
        2: {
          use catbinprodmor.
          3: apply identity.
          2: apply ((TransportedTensorComm Duniv H_eso H_ff TC) (x2,x3)).
        }
      }
    }
  ...
  Proof.
    ...
    2: {
      apply maponpaths_2.
      exact (! pr2 αD _ _   ((TransportedTensorComm Duniv H_eso H_ff TC (x1, x2)
  ...
  Proof.
    ...
    2: {
      ...
      exact (! pr2 αD _ _  ((id H (pr11 ((x1, TC (x2, x3)), x4)) #, TransportedTensorComm Duniv H_eso H_ff TC (x2, x3))
  ...
  Proof.
    ...
    2: {
      apply maponpaths_2.
      rewrite <- (functor_id TD).
      exact (! pr2 αD _ _  (id  (pr1 (HHH ((x1, x2), TC (x3, x4)))) #, TransportedTensorComm Duniv H_eso H_ff TC (x3, x4))).
    }
  ...
  Proof.
    ...
    rewrite TransportedAssocRightOnOb.
    rewrite TransportedAssocLeftInvOnOb.
    ...
    transparent assert (m : (E⟦(pr11 G) (TD (H c1, TD (H c2,, H c3))), pr11 G (H (TC (c1, TC (c2,, c3))))⟧)).
    {
      apply #(pr11 G).
      refine (_ · _).
      - use (#TD).
        2: {
          use catbinprodmor.
          3: apply identity.
          2: exact (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (c2,c3)).
        }
      -  apply (nat_z_iso_inv (TransportedTensorComm Duniv H_eso H_ff TC) (c1, TC (c2,c3))).
    }
    set (tt := cancel_precomposition _ _ _ _ _ _ m t).

    set (ptF := (TransportedTensorComm Duniv H_eso H_ff TC)).
    assert (pp : pt_GH (c1, c2)
                 = #(pr11 G) (ptH (c1,c2)) · ptG (H c1, H c2)).
    {
      apply idpath.
    }
